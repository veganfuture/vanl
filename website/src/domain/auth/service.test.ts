...
import { isSiteAdmin } from "./repository";
...
const { completeSignup, getSessionUser, inspectSignupToken, logout, startLogin, verifyLogin } =
...
describe("inspectSignupToken", () => {
  ...
  it("rejects a malformed token", async () => {
    expect(await inspectSignupToken("garbage")).toEqual({ error: "invalid" });
  });
});
...
describe("completeSignup", () => {
  it("creates a user, starts a session, and sets a cookie", async () => {
    ...
    expect(await getSessionUser(`${SESSION_COOKIE_NAME}=${cookieToken}`)).toMatchObject({
      accountName: "erin",
    });
  });
  ...
  it("grants site_admin when the account name is in the bootstrap list", async () => {
    ...
    if (!("user" in result)) throw new Error(`expected success, got ${JSON.stringify(result)}`);
    expect(await isSiteAdmin(result.user.id)).toBe(true);
  });
...
describe("login", () => {
  it("sends an OTP, then verifies it and starts a session", async () => {
    ...
    expect(await startLogin("heidi")).toEqual({ ok: true });
    const sentCode = vi.mocked(sendOtpViaBot).mock.calls[0][1];

    const result = await verifyLogin("heidi", sentCode);
  ...
  it("rejects an unknown account", async () => {
    expect(await startLogin("nobody")).toEqual({ error: "account_not_found" });
  });
  ...
  it("exhausts attempts after three wrong codes", async () => {
    ...
    await startLogin("ivan");

    expect(await verifyLogin("ivan", "0000")).toEqual({ error: "wrong_code" });
    expect(await verifyLogin("ivan", "0000")).toEqual({ error: "wrong_code" });
    expect(await verifyLogin("ivan", "0000")).toEqual({ error: "attempts_exhausted" });
  });
});
...
describe("session lifecycle", () => {
  it("logout revokes the session so it can no longer be used", async () => {
    ...
    expect(await getSessionUser(cookieHeader)).not.toBeNull();
    await logout(cookieHeader);
    expect(await getSessionUser(cookieHeader)).toBeNull();
  });

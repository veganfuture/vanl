  ...
  return (
    <div class="space-y-2">
      ...
      <div class="flex items-center gap-3">
        <label
          for={inputId}
          class="cursor-pointer rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-700"
        >
          {t("Afbeelding uploaden", "Upload image")}
        </label>
        <Show when={fileName()}>
          {(name) => <span class="truncate text-sm text-zinc-600">{name()}</span>}
        </Show>
      </div>

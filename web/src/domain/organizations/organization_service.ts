...
export type RemoveMemberError =
  "org_not_found" | "member_not_found" | "forbidden" | "sole_admin" | "internal_error";
export type ReplaceLogoError = "not_found" | "forbidden" | "validation" | "internal_error";
...
export class OrganizationService {
  ...
  /** Reuses requireOrgAdmin for authorization - same reasoning as EventService.replaceFlyer. */
  replaceLogo(
    actingUser: ActingUser,
    orgId: OrganizationId,
    bytes: Buffer,
  ): ResultAsync<Organization, ReplaceLogoError> {
    return this.requireOrgAdmin(actingUser, orgId, "not_found")
      .mapErr((error): ReplaceLogoError => error)
      .andThen(() =>
        processUpload(bytes, LOGO_VARIANTS)
          .mapErr((processingError): ReplaceLogoError => {
          ...
          .andThen(([full, thumbnail]) =>
            ResultAsync.combine([
              ...
              .mapErr((dbError): ReplaceLogoError => {
   ...
   * membership/profile-editing action. Public: replaceLogo reuses this

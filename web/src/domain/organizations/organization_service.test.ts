...
const service = new OrganizationService(repository, authRepository, imageRepository);
...
describe("replaceLogo", () => {
  it("an org_admin can upload a logo, producing two distinct variants of the shared thumbnail width", async () => {
    ...
    const org = (
      await service.createOrganization(actingAs(admin), baseInput({ name: "Logo Org" }))
    )._unsafeUnwrap();
  ...
  it("an org_editor cannot upload a logo", async () => {
    ...
    const org = (
      await service.createOrganization(actingAs(admin), baseInput({ name: "Editor Logo Org" }))
    )._unsafeUnwrap();
    (await repository.addMembership(org.id, editor, "org_editor"))._unsafeUnwrap();
  ...
  it("returns not_found when the org record is gone (requireOrgAdmin's DB existence check)", async () => {
    ...
    const org = (
      await service.createOrganization(actingAs(admin), baseInput({ name: "Deleted Logo Org" }))
    )._unsafeUnwrap();
    await sql`delete from organization_memberships where org_id = ${org.id.value}`;
    await sql`delete from organizations where id = ${org.id.value}`;
    const bytes = await makeJpeg(400, 400);

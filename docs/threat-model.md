...
| Compromised bot process abuses its website credential (new: Signal event ingestion) | The bot authenticates as a real but deliberately non-admin `signal-bot` account (`VANL_BOT_WEBSITE_API_TOKEN`, separate secret from `VANL_BOT_API_SHARED_SECRET` above) via the *same* public API and authorization rules as any human publisher — `EventService`'s permission checks, not a bypass. Worst case if this token leaks: the attacker can create/edit draft events and upload flyers as that one account, same blast radius as any single compromised publisher account — not `site_admin` power. The bot's *direct* Postgres access (`vanl_bot` role, for its own Signal message archive) is separately grant-scoped to only its own tables (`bot_archived_messages`, `bot_archived_attachments`, `bot_event_notifications`) — no grant on `events`/`users`/etc., so this credential leaking doesn't expose those tables either |

...
class BotApiServer:
    ...
    def __init__(self, config: BotApiConfig, client: SignalClient, env: BotEnv) -> None:
        self.config = config
        self.client = client
        self.env = env
    ...
    async def _handle_send_otp(self, request: Request) -> Response:
        ...
        if not hmac.compare_digest(auth_header, f"Bearer {expected_secret}"):
            logger.warning(
                "Rejected /messages/otp request with a bad or missing shared secret"
            )
        ...
        try:
            ...
            logger.warning(
                "Rejected /messages/otp request with an unparseable JSON body: {}", exc
            )
        ...
        if not OTP_CODE_RE.match(code):
            ...
            return JSONResponse(
                {"error": "code must be a 6-digit numeric string"}, status_code=400
            )
